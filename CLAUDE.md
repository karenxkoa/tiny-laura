...
El sitio es **catálogo + portafolio**, NO tienda con pasarela propia: el pago ocurre fuera
del sitio en **Ko-fi**. Diseño y desarrollo intermediario: karenxkoa (UX/UI en Figma).
...
- **Pago:** Ko-fi (externo). El redirect a Ko-fi ocurre a **nivel de producto**, no de categoría.
...
- **7 categorías navegables:** Descuentos, Tote Bags y T-Shirts, Prints, Planillas de Stickers,
  ...
  Todas (incluida Descuentos) tienen botón "Comprar" y redirigen a Ko-fi.
...
- Modal Ko-fi: `role="dialog"`, `aria-modal`, focus trap, cierre con Escape y clic en el fondo,
  y retorno de foco al botón que lo abrió.
...
- `catalogo.html` — grid de productos sin pills. **Sin enlaces entrantes** desde que la
  navegación apunta a `categorias.html`: decidir si se convierte en "todos los productos" o se borra.
...
- `app.js` — buscador (arriba y bottom nav) + modal Ko-fi accesible
...
- [ ] **Destino de `catalogo.html`:** quedó huérfano al mover la navegación a `categorias.html`.
      ¿Se convierte en "todos los productos" y se enlaza desde algún lado, o se elimina?
      Hoy comparte `<title>` "Catálogo" con `categorias.html`.
- [ ] **Assets reales:** sustituir placeholders (gatonejo, fotos de producto, polaroid) y el
      wordmark por el archivo de imagen definitivo.
- [ ] **URL real de Ko-fi:** hoy es `https://ko-fi.com/tinylaura` (placeholder) en `app.js` y enlaces.
